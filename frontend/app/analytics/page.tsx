...
import { Download, Save, TrashCan, Launch, Renew, Categories } from "@carbon/icons-react";
...
function NotConfigured() {
  return (
    <div style={{ padding: "3rem", textAlign: "center", color: "#525252" }}>
      <h2 style={{ marginBottom: "0.5rem" }}>Analytics not available</h2>
      <p>
        Configure <code>VITE_ANALYTICS_URL</code> to point at a running Granite.build
        analytics sidecar.
      </p>
    </div>
  );
...
export default function AnalyticsPage() {
  ...
  const [lastRunSource, setLastRunSource] = useState<"llm_phase1" | "llm_custom">("llm_phase1");
  ...
  async function handleRunCustom() {
    const categories = customInput.split(",").map((c) => c.trim()).filter(Boolean);
      ...
      >
        ...
        >
          <p style={{ marginBottom: "1rem", fontSize: "0.875rem", color: "#525252" }}>
            Enter comma-separated categories. Each failed build will be classified into one of them.
          </p>
          <TextInput
        ...
        {trendLoading ? (
          ...
          <>
            <FailureTrendChart data={trendData} daysBack={trendDays} isAnalyzing={daemonStatus?.analyzing} />
            {trendData ? (
                        ...
                        return (
                          ...
                          >
                            {(() => {
                              ...
                              const pageBuilds = builds.slice(start, start + CATEGORY_PAGE_SIZE);
                              return (
                                <Layer>
                                  <Table size="sm">
                                    <TableHead>
                                    ...
                                    <TableBody>
                                      {pageBuilds.map((b) => (
                                        <TableRow key={b.build_id}>
                                          <TableCell>
                                            <Link
                                              href={`/builds/${b.build_id}`}
                                              style={{ color: "var(--cds-link-primary, #0f62fe)" }}
                                            >
                                              {b.name}
                                            </Link>
                                          </TableCell>
                                          <TableCell style={{ color: "#525252", fontSize: "0.875rem" }}>
                                            {b.username}
                                          </TableCell>
                                          <TableCell style={{ color: "#525252", fontSize: "0.875rem" }}>
