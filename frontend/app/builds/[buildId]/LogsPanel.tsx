...
function AIBanner({
  ...
  useEffect(() => {
    if (!sidecarOk || !logContent || firedRef.current) return;
    firedRef.current = true;
    runAnalysis(logContent);
  }, [logContent, sidecarOk, runAnalysis]);
...
export function LogsPanel({ buildId, status }: Props) {
  ...
  const hasSidecar = isSidecarConfigured();
  ...
  return (
    ...
    >
      {hasSidecar && <AIBanner buildId={buildId} logContent={logContent} />}
      <div
        style={{
          display: "flex",
          flexDirection: "column",
