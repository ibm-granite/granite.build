...
def _make_space(name="public", resource_group_id=None):
    return StoredSpace(
        name=name,
...
class TestResolveSpaceResourceGroupId:
    def test_returns_cached_id_without_hf_call(self):
        ...
        space = _make_space(resource_group_id="cached-id")
        storage = MagicMock()
    ...
    def test_falls_back_to_hf_and_writes_back(self):
        ...
        space = _make_space(resource_group_id=None)
        storage = MagicMock()
        storage.space_storage.get_by_name.return_value = space
        ...
        assert space.resource_group_id == "resolved-id"
