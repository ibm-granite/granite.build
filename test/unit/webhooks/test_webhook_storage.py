...
class TestWebhookStorage:
    ...
    def test_add_and_get(self):
        ...
        assert retrieved.build_filter == "build-001"
    ...
    def test_get_active_for_build_filter(self):
        """Test filtering active subscriptions by build_filter.
        ...
        active_sub = self._make_subscription(
            build_filter=build_id, active=True, space_name="space-a"
        )
        inactive_sub = self._make_subscription(
            build_filter=build_id, active=False, space_name="space-a"
        )
        other_build_sub = self._make_subscription(
            build_filter=other_build_id, active=True, space_name="space-a"
        )
        ...
        results = self.storage.get_active_for_build_filter(build_id)
        assert len(results) == 1
        assert results[0].uuid == active_sub.uuid
        assert results[0].build_filter == build_id
        assert results[0].active is True

    def test_deactivate(self):
        """Test deactivating a single subscription by ID."""
        sub = self._make_subscription(
            active=True, build_filter=f"build-deact-{uuid.uuid4().hex[:8]}"
        )
    ...
    def test_deactivate_for_build(self):
        ...
        Add 2 active subs for same build_filter, deactivate_for_build returns 2.
        """
        build_id = f"build-deactall-{uuid.uuid4().hex[:8]}"
        other_build_id = f"build-keep-{uuid.uuid4().hex[:8]}"

        sub1 = self._make_subscription(build_filter=build_id, active=True)
        sub2 = self._make_subscription(build_filter=build_id, active=True)
        sub_other = self._make_subscription(build_filter=other_build_id, active=True)
    ...
    def test_get_active_for_space(self):
        ...
        Setup: 1 space-wide sub (build_filter=None) in target-space, 1 per-build
        ...
        # build_filter may be None or "" depending on storage layer serialization
        assert results[0].build_filter is None or results[0].build_filter == ""
    ...
    def test_get_by_space(self):
        ...
        sub_a1 = self._make_subscription(space_name=space_a, build_filter="b1")
        sub_a2 = self._make_subscription(space_name=space_a, build_filter="b2")
        sub_b = self._make_subscription(space_name=space_b, build_filter="b3")
