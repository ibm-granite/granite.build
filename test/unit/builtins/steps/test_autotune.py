"""Structural unit tests for the `autotune` step asset (bash).

The docker variant is deferred to the feat/autotune-step-docker branch, which
carries its own tests plus FINDINGS.md (an unresolved artifact-delivery issue).
"""
...
class TestStepsSourceTree:
    ...
    def test_source_exists_with_framework_files(self):
        src, published = BASH_SRC, BASH_STEP
    ...
    def test_published_matches_source(self):
