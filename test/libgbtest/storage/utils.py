...
class TargetSpec(BaseModel):
    ...
    def show_connections(self):
        ...
        print(
            f"Step   {self.step.uuid} connected to build {self.step.build_id}, target {self.step.target_id}"
        )
        for art in self.input_artifacts:
            print(
                f" Input artifact {art.uuid} connected to build {art.created_by_build_id}, target {art.created_by_target_id}, step {art.created_by_step_id}"
            )
        for art in self.output_artifacts:
            print(
                f"Output artifact {art.uuid} connected to build {art.created_by_build_id}, target {art.created_by_target_id}, step {art.created_by_step_id}"
            )
...
def _store_connected_build(
    build: StoredBuild, targets: list[TargetSpec], storage: StorageCollection
):
...
def connect_and_store_build(
    build: StoredBuild,
    targets: Union[TargetSpec, list[TargetSpec]],
