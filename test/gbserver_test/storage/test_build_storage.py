...
class BuildStorageTestSupport(AbstractStorageTestSupport):
    ...
    def _get_build_user(self) -> str:
        ...
        if token is None:
            ...
            assert token is not None, "Need a github token to create the correct username for the build"
    ...
    def _get_test_item(self, index):
        ...
        obj = StoredBuild(name=f"bname{index}", space_name=f"myspace{index}", source_uri=f"https://git.ibm.com/{index}", username=username,
                          build_archive="", description=f"description{index}", targets=[f"foo{index}"], tags=[f"tag{index}", f"tag{index+1000}"])
        return obj
