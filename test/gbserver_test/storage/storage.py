...
from gbserver.storage.storage import CREATED_TIME_FIELD_NAME, JSON_COLUMN_NAME, UPDATED_TIME_FIELD_NAME, UUID_COLUMN_NAME, BaseItemStorage, BaseStoredItem, Pagination, QueryControl, SortOrder, TaggedItem
from gbserver.utils.logger import get_logger
from gbserver.utils.utils import get_differing_attributes
from gbserver_test.test_utils import AbstractReadonlySingletonStorageUsingTest, AbstractSingletonStorageUsingTest
...
class AbstractStorageTestSupport():
    ...
    def _get_test_item(self,index:int) -> BaseStoredItem:
        raise NotImplemented("Sub-classes must implement this to create instances to be stored in the test storage")
...
class AbstractStorageTest(AbstractSingletonStorageUsingTest):
    ...
    def _get_tested_storage(self) -> BaseItemStorage:
        raise NotImplemented("Sub-classes must implement this to get the storage under test from self.storage") 
    ...
    def test_add(self):
        ...
        try:
            ...
            logger.info("Storage may have issued a WARNING or ERROR above for adding a duplicate uuid and can safely be ignored.")
            pass
    ...
    def test_search_by_time(self):
        ...
        if item0_created_time is None and item0_updated_time is None: 
            pytest.skip(f"Stored items contain neither {CREATED_TIME_FIELD_NAME} or {UPDATED_TIME_FIELD_NAME} fields.")
            return
    ...
    def test_update(self):
        ...
        if hasattr(item1,CREATED_TIME_FIELD_NAME):
            setattr(updated_item1,CREATED_TIME_FIELD_NAME, getattr(item1,CREATED_TIME_FIELD_NAME))
        if hasattr(item1,UPDATED_TIME_FIELD_NAME):
            setattr(updated_item1,UPDATED_TIME_FIELD_NAME, getattr(item1,UPDATED_TIME_FIELD_NAME))
        assert updated_item1 != item1, "the items should have different values for this test to be valid"
        storage.update(updated_item1, update_updated_time=False)   # Update item0 with item1 (which now has item0's uuid)
        items = storage.get_by_uuid(item1.uuid)
        self._verify_get_results(items, updated_item1) # Verify that the item stored under item0's uuid == item1
    ...
    def test_updated_time(self):
        ...
        if not hasattr(item, UPDATED_TIME_FIELD_NAME):
            pytest.skip(f"Items do not include an {UPDATED_TIME_FIELD_NAME} field so skipping this test")
            return
    ...
    def _get_where_search_columns(self, storage:BaseItemStorage, item:BaseStoredItem) ->  dict[str, Any]:
    ...
    def _duplication_test_helper(self, field_names:list[str]):
        ...
        """
        assert len(field_names) > 0 , "This helper test method must be called with 1 or 2 field names"
        ...
        for field_name in field_names:
            ...
            assert item1_value is not None, f"Item does not appear to have the expected field {field_name}"
            setattr(item2a,field_name, item1_value)
            if item2b and not first:
                setattr(item2b,field_name, item1_value) # Make all but the first field the same.  We should be able to add this one.
            first = False
        storage = self._get_tested_storage()
        ...
        try:
            ...
            logger.info(f"The test may have logged an expected exception above for the addition of a duplicate item having common values for {field_names} field(s) ")
            assert storage.get_by_uuid(item2a.uuid) is None, "Found item with duplicate value in storage"
        ...
        try:
            ...
            logger.info(f"The test may have logged an expected exception above for the upsert of a duplicate item having common values for {field_names} field(s) ")
            assert storage.get_by_uuid(item2a.uuid) is None, "Found item with duplicate value in storage"
    ...
    def test_update_fields(self):
        ...
        for name,value in item0_attributes.items():
            if name not in [UPDATED_TIME_FIELD_NAME, CREATED_TIME_FIELD_NAME,UUID_COLUMN_NAME] and not name.startswith("_"):
                matches[name] = value
        ...
        for name,value in updates.items():
            assert updated_item0_attributes[name] == value, "Updated attribute {name} did not get assigned value {value}"
