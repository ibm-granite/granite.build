...
from gbserver_test.buildwatcher.buildtest import AbstractBuildTest, BuildTestSpecification, ClassTestedEnum, ExpectedTarget
from gbserver_test.constants import GBTEST_SPACE_NAME, GBTEST_USER_NAME
...
class TestShouldRetry:
    ...
    def test_non_failed_build_not_retried(self):
        runner = self._runner()
        for st in (Status.SUCCESS, Status.PENDING, Status.RUNNING, Status.CANCELLED, Status.INVALID):
    ...
    def test_failed_with_max_retries_first_attempt(self):
        build = _make_stored_build_with_config(_BUILD_YAML_MAX_RETRIES_2, status=Status.FAILED, retry_count=0)
        assert self._runner()._should_retry(build)

    def test_failed_with_max_retries_second_attempt(self):
        build = _make_stored_build_with_config(_BUILD_YAML_MAX_RETRIES_2, status=Status.FAILED, retry_count=1)
        assert self._runner()._should_retry(build)

    def test_failed_exhausted_retries(self):
        build = _make_stored_build_with_config(_BUILD_YAML_MAX_RETRIES_2, status=Status.FAILED, retry_count=2)
        assert not self._runner()._should_retry(build)
    ...
    def test_retry_build_preserves_lineage(self):
        ...
        build = _make_stored_build_with_config(
            _BUILD_YAML_MAX_RETRIES_2, status=Status.FAILED, retry_count=1, retry_of_build_id=original_id
        )
...
class TestBuildRunnerRetry(AbstractBuildTest):
    ...
    def _wait_for_second_build(self,original_build_id, timeout_seconds):
        self._wait_for(fn=self._has_retried_build,
    ...
    def test_buildrunner_retry(self: Self):
        ...
        assert original.status == Status.FAILED, self._failed_build_msg(original_id, f"Original build status: {original.status}")
        assert original.retry_build_id == retry_id, self._failed_build_msg(original_id, "Original build should point to retry")
        assert original.retry_of_build_id is None, self._failed_build_msg(original_id, "Original build should not have a retry_of_build_id")

        retry = self.storage.build_storage.get_by_uuid(retry_id)
        assert isinstance(retry, StoredBuild)
        assert retry.retry_of_build_id == original_id, self._failed_build_msg(retry_id, "Retry build should point back to original")
        assert retry.retry_count == 1, self._failed_build_msg(retry_id, f"Expected retry_count=1, got {retry.retry_count}")
        assert retry.retry_build_id is None, self._failed_build_msg(retry_id, "Retry build should not itself have been retried")
        assert retry.source_uri != "", self._failed_build_msg(retry_id, "Retry build should have a new PR source_uri")
        assert retry.source_uri != original.source_uri, self._failed_build_msg(retry_id, "Retry build should have a different source_uri from the original")
        ...
        assert len(original_targets) > 0, self._failed_build_msg(original_id, "Expected targets in original build")
        for original_target in original_targets:
            ...
            assert len(retry_targets) == 1, self._failed_build_msg(
                retry_id, f"Expected exactly one retry target named '{original_target.name}'"
            )
            retry_target = retry_targets[0]
            assert isinstance(retry_target, StoredTargetRun)
            assert retry_target.skipped_for_prerun_target_id == original_target.uuid, self._failed_build_msg(
                retry_id,
        ...
        assert len(retry_events) > 0, self._failed_build_msg(retry_id, "Expected events for retry build")
