...
class AbstractAPITest(AbstractSingletonStorageUsingPreloadedSpaceTest):
    ...
    def get_gh_username(self, token:str=GBSERVER_GITHUB_TOKEN) -> str:
        ...
        assert user is not None and isinstance(user,User), "Could not get username/login from git token"
        username = user.login
        return username
    ...
    def get_test_client(token:str=GBSERVER_GITHUB_TOKEN) -> TestClient:
        ...
        if token == None or token == "":
            ...
            assert token != None, "GBSERVER_GITHUB_TOKEN or GITHUB_TOKEN env var must be set to enable server authentication"
        client = TestClient(root_api, headers={
