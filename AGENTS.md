...
- **Frontend**: `git subtree split --prefix=frontend/packages/ui-core` produces a `dist/ui-core` branch, tagged per release, that `gb-ui`'s `package.json` depends on via yarn's git shorthand.
