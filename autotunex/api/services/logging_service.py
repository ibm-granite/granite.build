...
class DatabaseProtocol(Protocol):
    """Protocol defining the interface for database operations (sync variants for thread contexts)."""

    def create_logging_table_sync(self) -> None:
        """Create the logging table if it doesn't exist."""
        ...

    def insert_logs_sync(self, buffer: list) -> None:
...
class BufferedLogHandler(logging.Handler):
    def __init__(
        ...
        if self.db is not None:
            self.db.create_logging_table_sync()
    ...
    def _flush_to_database(self):
        ...
        if self.db is not None:
            ...
            self.db.insert_logs_sync(buffer=db_buffer)
    ...
    def flush(self):
        ...
        try:
            ...
            self.buffer = []  # Prevent unbounded growth on repeated failures
    ...
    def __getstate__(self):
        # Remove the lock from the object's state for pickling
