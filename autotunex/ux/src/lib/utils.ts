...
export class Utils {
	...
	static processUploadedFile(file: File, maxLines?: number): Promise<DatasetType[]> {
		return new Promise((resolve, reject) => {
			...
			if (file.name.endsWith('.parquet')) {
				// Parquet is binary — read as ArrayBuffer and parse with hyparquet
				const binReader = new FileReader();
				binReader.onerror = reader.onerror;
				binReader.onload = async (event: any) => {
					try {
						const { parseParquet } = await import('$lib/file-parser');
						const rows = await parseParquet(event.target.result as ArrayBuffer, maxLines);
	...
	static async countLinesInFile(file: File): Promise<number> {
		...
		if (file.name.endsWith('.parquet')) {
			try {
				const { countParquetRows } = await import('$lib/file-parser');
				return await countParquetRows(await file.arrayBuffer());
			} catch {
		...
		return new Promise((resolve, reject) => {
			...
			reader.onload = (event: any) => {
				...
				try {
					...
					if (isParquet && content instanceof ArrayBuffer) {
						worker.postMessage(message, [content]);
					} else {
						worker.postMessage(message);
					}
				} catch {
					// If Worker creation fails, fall back to sync
