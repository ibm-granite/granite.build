...
export const publishedModels = writable<Model[]>();
 ...
 * Fetches the first page every 5s and replaces the top of the store,
...
export const fetchAndCacheTrialLogs = async (
	...
	const result = await api.getTrialLogs(trialId, 0, 50);
	const logs = result.logs ?? [];
	const oldestId = logs.length > 0 ? logs[logs.length - 1].id ?? 0 : 0;
...
export const loadOlderTrialLogs = async (trialId: string): Promise<void> => {
	const cached = getCachedTrialLogs(trialId);
	if (!cached || !cached.hasMore || loadingOlderTrialLogs.has(trialId)) return;

	loadingOlderTrialLogs.add(trialId);

	const result = await api.getTrialLogs(trialId, cached.oldestId, 50);
	const newLogs = result.logs ?? [];
	const oldestId = newLogs.length > 0 ? newLogs[newLogs.length - 1].id ?? 0 : cached.oldestId;
...
export const startTrialLogPoll = async (trialId: string) => {
	if (activeTrialLogPolls.has(trialId)) return;

	await fetchAndCacheTrialLogs(trialId, { status: 'RUNNING' });
