...
<script lang="ts">
	...
	export let additionalInfo: any = null;
	...
	let hasFetchedDefaultDmfModel = false;
	...
	$: if (modelSource !== previousModelSource) {
		...
		if (modelSource === ModelSource.DMF) {
			selectedModel = 'granite-4.0-micro';
	...
	async function fetchDefaultDmfModel() {
		try {
			isSearching = true;
			const dmfModels = await api.searchDMFModels('granite-4.0-micro');
			if (Array.isArray(dmfModels?.data) && dmfModels.data.length > 0) {
				const uniqueModels = new Map();
				dmfModels.data.forEach((model: any) => {
					if (!model.namespace || !model.base_model || !model.revision) return;
					if (model.base_model.includes('"') || model.base_model.includes('/')) return;
					...
					if (!uniqueModels.has(modelId)) {
						uniqueModels.set(modelId, { id: modelId, text: label, isOpen, rawModel: model });
					}
				});
				suggestions = Array.from(uniqueModels.values());
				const defaultModel =
					dmfModels.data.find(
						(m: any) => m.model_id === 'granite-4.0-micro' || m.base_model === 'granite-4.0-micro'
					) || dmfModels.data[0];
				selectedDmfModel = {
				...
				additionalInfo = defaultModel;
			} else {
				suggestions = [{ id: 'granite-4.0-micro', text: 'granite-4.0-micro' }];
			}
			isSearching = false;
			hasFetchedDefaultDmfModel = true;
		} catch (error) {
			console.error('Error fetching DMF model:', error);
			suggestions = [{ id: 'granite-4.0-micro', text: 'granite-4.0-micro' }];
			isSearching = false;
			hasFetchedDefaultDmfModel = true;
		}
	...
	async function fetchModelCard() {
		try {
			if (modelSource === ModelSource.DMF || !selectedModel) {
				modelCard = null;
				return;
			}
										...
										on:select={(e) => {
											if (e.detail.selectedItem?.id) {
												...
												if (modelSource === ModelSource.DMF) {
													...
													additionalInfo = e.detail.selectedItem.rawModel;
...
<!-- Model card modal -->
<Modal
	bind:open={showModelCardModal}
	modalHeading={modelSource === ModelSource.DMF ? 'Model Details' : 'Model Card'}
	passiveModal
	size="lg"
>
	{#if modelSource === ModelSource.DMF && selectedDmfModel?.rawModel}
		<div class="model-info">
			<div class="info-row">
				<span class="info-label">Model ID:</span> <span>{selectedDmfModel.rawModel.model_id}</span>
			</div>
			<div class="info-row">
			...
			<div class="info-row">
				<span class="info-label">Revision:</span> <span>{selectedDmfModel.rawModel.revision}</span>
			</div>
		</div>
	{:else if modelCard?.code}
