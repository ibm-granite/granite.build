...
<script lang="ts">
	...
	function prepareReviewStep() {
		...
		// Non-blocking resource estimation
		if (selectedModel && selectedConfigId) {
			api
				.estimateUsage({
					model_name: selectedModel,
					config_id: selectedConfigId === '__pending__' ? '' : selectedConfigId,
					gpu_memory: 80
				})
				.then((res) => {
	...
	async function handleLaunch() {
		...
		try {
			...
			transitionError = err.message || 'Launch failed. Please try again.';
		} finally {
			isLaunching = false;
			launchPhase = null;
			uploadProgress = 0;
