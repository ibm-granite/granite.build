...
<script lang="ts">
	...
	import { CodeSnippet, InlineNotification, ProgressBar } from 'carbon-components-svelte';
...
{:else if !isLoading && tasks?.length > 0}
	...
	>
		<svelte:fragment slot="cell" let:cell let:row>
			...
			{:else if cell.key === 'pr_url'}
				<CodeSnippet
					code={row?.pr_url}
					type="inline"
					style="background: transparent; word-wrap: break-word;"
				/>
