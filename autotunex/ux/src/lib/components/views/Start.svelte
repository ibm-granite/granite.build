...
<script lang="ts">
	...
	import { Login, ModelTuned, Settings } from 'carbon-icons-svelte';
	import Resources from '$lib/components/views/Resources.svelte';
	import { isAuthenticated, currentUser } from '$lib/store';
	import { API } from '$lib/api';

	const Radu = new URL('/static/radu.png', import.meta.url).href;
	const Daniel = new URL('/static/daniel.png', import.meta.url).href;
	const Priyanshu = new URL('/static/priyanshu.png', import.meta.url).href;
	const Fred = new URL('/static/fred.png', import.meta.url).href;
	const Teryl = new URL('/static/teryl.png', import.meta.url).href;
							...
							{:else}
								<Button
									...
									on:click={async () => {
										...
										console.log('🚀 ~ login:', login.authorize);
										if (login?.authorize) {
											window.location.href = login.authorize;
										}
									}}>Login</Button
								>
							{/if}
...
{#if innerWidth > 800 && innerHeight - 800 > 0}
	...
	>
		Copyright &copy;2025 by IBM Corp. All rights reserved.
	</div>
{/if}
