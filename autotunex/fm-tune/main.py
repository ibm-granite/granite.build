...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--fleet",
        type=str,
        choices=["ccc-a100", "bv-h100"],
        default="ccc-a100",
        help=(
            "Deployment fleet. Selects the GPU model and the per-fleet default "
            "for NCCL_IB_HCA. 'ccc-a100' (default): A100, single rail (mlx5_0). "
            "'bv-h100' (BlueVela H100): H100, 8 compute rails (mlx5_0..mlx5_7). "
    ...
    try:
        ...
        if handler:
            try:
                handler.record_data(
                    {"id": handler.get_job_id(), "status": "COMPLETED"},
                    record_type=RecordType.UPDATE_STATUS,
                )
            except Exception as e:
                logger.warning(f"[AutoTune] Failed to report COMPLETED status: {e}")
    except Exception as e:
        logger.error(f"[AutoTune] Training failed: {e}", exc_info=True)
        if handler:
            try:
                handler.record_data(
                    {"id": handler.get_job_id(), "status": "ERROR"},
                    record_type=RecordType.UPDATE_STATUS,
                )
