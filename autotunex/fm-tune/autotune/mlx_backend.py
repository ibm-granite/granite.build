...
def ensure_mlx_model(model_path: str, quantize: bool, cache_dir: str = None) -> str:
    ...
    try:
        logger.info(
            f"[AutoTune][MLX] Converting {model_path} -> MLX (quantize={quantize})"
        )
        convert(
            hf_path=model_path,
            mlx_path=tmp_model,
...
def translate_config(
    training_config: dict, params: dict, epochs: int, n_train_examples: int
) -> dict:
    ...
    return {
        ...
        "grad_accumulation_steps": int(
            params.get("gradient_accumulation_steps", 1) or 1
        ),
...
def build_records(inputs: list, outputs: list):
    ...
    if isinstance(inputs[0], list):
        recs = [
            {"messages": list(i) + [{"role": "assistant", "content": o}]}
...
def _build_lr(lr: float, iters: int, warmup: int, kind: str):
    ...
    try:
        ...
        logger.warning(
            f"[AutoTune][MLX] LR schedule build failed ({e}); using constant lr."
        )
        return lr
...
def run_training(
    mlx_model_path, mlx_cfg, train_records, is_chat, eval_records, adapter_dir
) -> dict:
    ...
    lr = _build_lr(
        mlx_cfg["learning_rate"],
        mlx_cfg["iters"],
        mlx_cfg["warmup"],
        mlx_cfg["lr_scheduler_type"],
    )
...
def save_output(run_result: dict, mlx_cfg: dict, dest: str) -> None:
    ...
    if mlx_cfg["fine_tune_type"] == "full":
        ...
        logger.info(
            f"[AutoTune][MLX] Saved full MLX weights to {dest} (MLX-native, not PEFT)."
        )
    else:
        ...
        try:
            ...
            save_config(
                _adapter_config(mlx_cfg), os.path.join(dest, "adapter_config.json")
            )
        ...
        logger.info(
            f"[AutoTune][MLX] Saved MLX-native LoRA adapter to {dest} (not PEFT)."
        )
