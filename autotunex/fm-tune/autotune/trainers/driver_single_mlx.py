...
def train_driver_single_gpu(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    epochs = (
        training_config.get("hpo_num_epochs", 1)
        if hpo_search
        else training_config.get("num_train_epochs", 1)
    )

    mlx_model_path = mlx_backend.ensure_mlx_model(
        model_name_or_path, quantize=(algo == "qlora")
    )
    mlx_cfg = mlx_backend.translate_config(
        training_config, params, epochs=epochs, n_train_examples=len(train_records)
    )

    adapter_dir = os.path.join(output_dir, "mlx_adapters", str(trial_id))
    run_result = mlx_backend.run_training(
