...
def _load_tokenize_and_save(
    ...
    num_rows = len(table)
    with tqdm(
        total=num_rows, desc=f"Writing {dataset_name}.arrow", unit="rows"
    ) as pbar:
...
def train_loop_per_worker(train_loop_config: Dict[str, Any]):
    ...
    training_args_kwargs = dict(
        ...
        gradient_checkpointing_kwargs={
            "use_reentrant": deepspeed_config.get("zero_optimization", {}).get(
                "stage", 0
            )
            == 3,
        },
...
def train_driver_multi_gpu(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"[AutoTune] Distributed training finished with result: {train_result}"
        )
