...
def _mps_accelerator() -> "Accelerator":
    ...
    bf16 = torch.backends.mps.is_available() and torch.backends.mps.is_macos_or_newer(
        14, 0
    )
    return Accelerator(
        kind="mps",
...
def detect_accelerator() -> "Accelerator":
    ...
    if override:
        if override not in _VALID_OVERRIDES:
            raise ValueError(
                f"FMTUNE_DEVICE must be one of {sorted(_VALID_OVERRIDES)}; got {override!r}."
            )
        if override == "cuda":
...
def resolve_precision(
    requested: str, accel: "Accelerator", probe_autocast: bool = True
) -> str:
    ...
    if accel.kind == "mps":
        ...
        if probe_autocast and not supports_autocast_bf16():
            logger.warning(
                "[AutoTune] bf16 requested but MPS autocast unavailable; using fp32."
            )
            return "fp32"
        return "bf16"
    return "fp32"  # cpu
...
def apply_platform_guards(
    ...
    if backend == "mlx":
        ...
        if rl_algo and rl_algo != "none":
            raise ValueError(
                f"RL fine-tuning ('{rl_algo}') is not supported on the MLX backend."
            )
        ...
        if mct > 1:
            logger.warning(
                f"[AutoTune] max_concurrent_trials={mct} would oversubscribe the single "
                "MPS device; clamping to 1."
            )
    ...
    if fixed_attn != attn:
        logger.warning(
            f"[AutoTune] use_flash_attention='{attn}' unavailable on {accel.kind}; using '{fixed_attn}'."
        )
    ...
    if mct > 1:
        logger.warning(
            f"[AutoTune] max_concurrent_trials={mct} would oversubscribe the single {accel.kind} "
            "device; clamping to 1."
        )
        tune_config["max_concurrent_trials"] = 1
    ...
    if fixed_prec != prec:
        logger.warning(
            f"[AutoTune] precision='{prec}' unavailable on {accel.kind}; using '{fixed_prec}'."
        )
...
def object_store_bytes() -> int:
    ...
    if val:
        try:
            ...
            logger.warning(
                f"[AutoTune] FMTUNE_OBJECT_STORE_BYTES={val!r} is not an int; using default."
            )
