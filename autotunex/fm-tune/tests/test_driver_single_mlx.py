...
def _patch_backend(monkeypatch, calls):
    ...
    monkeypatch.setattr(
        drv.mlx_backend,
        "ensure_mlx_model",
        lambda model, quantize, **k: calls.setdefault("ensure", (model, quantize))
        or "/mlx/model",
    )
    monkeypatch.setattr(
        drv.mlx_backend,
        "translate_config",
        lambda tc, params, epochs, n_train_examples: calls.setdefault(
            "translate",
            {
                "epochs": epochs,
                "n": n_train_examples,
                "algo": tc.get("tuning_algorithm"),
            },
        )
        or {
            "fine_tune_type": "lora",
            "quantize": tc.get("tuning_algorithm") == "qlora",
        },
    )
    monkeypatch.setattr(
        drv.mlx_backend,
        ...
        lambda inputs, outputs: (
            [{"prompt": inputs[0], "completion": outputs[0]}],
            False,
        ),
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        drv.mlx_backend,
        "save_output",
        lambda res, cfg, dest: saved.update({"dest": dest}),
    )
    calls["saved"] = saved
