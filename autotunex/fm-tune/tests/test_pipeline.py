...
class TestValidCombinations:
    def test_sft_only(self):
        p = AutotunePipeline(
            tuning_algo="sft", rl_algo="none", model_name_or_path=MODEL
        )
    ...
    def test_lora_only(self):
        p = AutotunePipeline(
            tuning_algo="lora", rl_algo="none", model_name_or_path=MODEL
        )
    ...
    def test_qlora_only(self):
        ...
        p = AutotunePipeline(
            tuning_algo="qlora", rl_algo="none", model_name_or_path=MODEL
        )
    ...
    def test_qlora_with_dpo(self):
        p = AutotunePipeline(
            tuning_algo="qlora", rl_algo="dpo", model_name_or_path=MODEL
        )
        assert p.tuning_algo == "qlora"
    ...
    def test_lora_with_dpo(self):
        ...
        p = AutotunePipeline(
            tuning_algo="lora", rl_algo="dpo", model_name_or_path=MODEL
        )
        assert p.tuning_algo == "lora"
        assert p.rl_algo == "dpo"
    ...
    def test_lora_with_kto(self):
        p = AutotunePipeline(
            tuning_algo="lora", rl_algo="kto", model_name_or_path=MODEL
        )
    ...
    def test_online_rl_resets_tuning_to_none(self):
        ...
        p = AutotunePipeline(
            tuning_algo="lora", rl_algo="ppo", model_name_or_path=MODEL
        )
    ...
    def test_grpo_alone(self):
        p = AutotunePipeline(
            tuning_algo="none", rl_algo="grpo", model_name_or_path=MODEL
        )
        assert p.rl_algo == "grpo"

    def test_dapo_alone(self):
        p = AutotunePipeline(
            tuning_algo="none", rl_algo="dapo", model_name_or_path=MODEL
        )
    ...
    def test_none_string_treated_as_none(self):
        ...
        p = AutotunePipeline(
            tuning_algo="None", rl_algo="ppo", model_name_or_path=MODEL
        )
        assert p.tuning_algo == "none"
...
class TestInvalidCombinations:
    def test_both_none_raises(self):
        with pytest.raises(ValueError, match="cannot be `none`"):
            AutotunePipeline(
                tuning_algo="none", rl_algo="none", model_name_or_path=MODEL
            )

    def test_offline_rl_without_tuning_raises(self):
        with pytest.raises(ValueError, match="Offline RL"):
            AutotunePipeline(
                tuning_algo="none", rl_algo="dpo", model_name_or_path=MODEL
            )

    def test_unknown_tuning_algo_raises(self):
        with pytest.raises(AssertionError):
            AutotunePipeline(
                tuning_algo="garbage", rl_algo="none", model_name_or_path=MODEL
            )

    def test_unknown_rl_algo_raises(self):
        with pytest.raises(AssertionError):
            AutotunePipeline(
                tuning_algo="sft", rl_algo="garbage", model_name_or_path=MODEL
            )
