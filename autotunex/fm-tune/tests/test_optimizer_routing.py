...
def _select_driver(
    multi_gpu: bool, rl_algo: str, train_implementation: str, backend: str = "torch"
) -> str:
    ...
    if not multi_gpu:
        if rl_algo in AUTOTUNE_OFFLINE_RL:
            ...
            return (
                "autotune.trainers.driver_single_mlx"
                if backend == "mlx"
                else "autotune.trainers.driver_single"
            )
    # multi-gpu
...
def test_single_device_backend_routing():
    ...
    assert (
        _select_driver(False, "none", "FSDP", backend="torch")
        == "autotune.trainers.driver_single"
    )
    # MLX backend -> the MLX single-device driver.
    assert (
        _select_driver(False, "none", "FSDP", backend="mlx")
        == "autotune.trainers.driver_single_mlx"
    )
    # backend does not affect multi-GPU or RL routing.
    assert (
        _select_driver(True, "none", "FSDP", backend="mlx")
        == "autotune.trainers.driver_multi_hf_fsdp"
    )
    assert (
        _select_driver(False, "dpo", "DeepSpeed", backend="mlx")
        == "autotune.trainers.driver_single_trl"
    )


def test_mlx_driver_module_importable():
    # The module the mlx branch imports must exist and expose the driver fn.
    from autotune.trainers.driver_single_mlx import (  # noqa: F401
