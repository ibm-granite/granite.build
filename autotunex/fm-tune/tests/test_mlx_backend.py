...
def test_cache_key_reflects_model_and_quant():
    ...
    assert (
        cache_key("HuggingFaceTB/SmolLM2-135M-Instruct", True)
        == "SmolLM2-135M-Instruct__q4"
    )
...
def _install_fake_mlx_lm(monkeypatch, record):
    ...
    def convert(
        hf_path, mlx_path, quantize=False, q_bits=None, q_group_size=None, **kw
    ):
        record.append(
            {
                "hf_path": hf_path,
        ...
        os.makedirs(
            mlx_path
        )  # NOT exist_ok=True -- mirror real mlx_lm.convert's precondition
...
def test_translate_config_iters_never_zero():
    ...
    params = {
        "per_device_train_batch_size": 999,
        "learning_rate": 1e-5,
        "r": 8,
        "alpha_ratio": 2.0,
    }
...
def _install_full_fake_mlx(monkeypatch, recorder):
    ...
    core.save_safetensors = lambda path, d: recorder.setdefault("saved", []).append(
        path
    )
    ...
    mlx_mod.core, mlx_mod.nn, mlx_mod.optimizers, mlx_mod.utils = (
        core,
        nn,
        opts,
        utils_mod,
    ...
    lm_utils.save_config = lambda cfg, path: recorder.setdefault("saved", []).append(
        path
    )
    ...
    def train(
        model, args, optimizer, train_dataset, val_dataset, training_callback=None
    ):
    ...
    tuner_utils.linear_to_lora_layers = lambda *a, **k: recorder.setdefault(
        "lora_call", a
    )
...
def test_run_training_wires_mlx_and_returns_losses(tmp_path, monkeypatch):
    ...
    out = run_training(
        str(tmp_path), cfg, recs, False, recs, str(tmp_path / "adapters")
    )
    ...
    assert (
        rec["targs"]["batch_size"] == 1
    )  # clamped from 2 to the 1-record dataset size
