...
async def get_current_user(request: Request) -> base_models.AuthUser:
    if not OIDC_ENABLED:
        ...
        email = _email_from_request(request) or os.getenv(
...
@prefix_router.post("/api/record_logs", tags=["Utils"])
...
@prefix_router.post("/api/record_trial", tags=["Utils"])
...
@prefix_router.post("/api/update_status", tags=["Utils"])
...
@prefix_router.post("/api/insert_trial_result", tags=["Utils"])
