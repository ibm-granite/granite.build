...
class Config:
        ...
        try:
            if config.id is not None:
                ...
                if (
                    ...
                    and existing_config.get("name", None) == config.name
                ):
                    logger.error(
                        f"Configuration '{config.name}' already exists for this user"
                    )
                    raise Exception(
                        f"Configuration '{config.name}' already exists for this user"
                    )
    ...
    def get_config_by_name(self, user_id: str, config_name: str) -> dict | None:
        ...
        return self.db.get_config_by_name_and_user(
            config_name=config_name, user_id=user_id
        )
    ...
    def find_or_create(
        ...
        existing = self.db.get_config_by_name_and_user(
            config_name=name, user_id=user_id
        )
        if existing is None:
            return _insert(name)
        ...
        for suffix in range(1, MAX_CONFIG_SUFFIX + 1):
            ...
            existing = self.db.get_config_by_name_and_user(
                config_name=candidate, user_id=user_id
            )
            if existing is None:
