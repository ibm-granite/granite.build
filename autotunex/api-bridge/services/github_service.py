...
class GitHubService:
    ...
    async def get_gb_user_details(self, build_id) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                f"Exception occured in get_gb_user_details: {e}", exc_info=True
            )
            return {"error": str(e)}
    ...
    def find_pr_by_build_id(
        ...
        if not response.ok:
            raise Exception(
                f"GitHub Search API error: {response.status_code} {response.text}"
            )
    ...
    def extract_username(self, text: str) -> str | None:
        ...
        Example inputs:
            "run: user Priyanshu-Rai build d7cc51ba-efd0-4ef0-bab6-19aa826745ec autotunex"
            "run: user `Priyanshu-Rai` build `ff76a26e-e18c-44d6-9d37-15c09c0293ab` `autotunex`"

        Returns:
            The username string (e.g. "Priyanshu-Rai"), or None if not found.
        """
