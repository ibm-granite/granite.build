...
cicd-pr-test: 
	...
	source $(VENVDIR)/bin/activate && \
		...
		pytest --durations=20 --cov --cov-report=xml --junitxml=report.xml -n ${PYTEST_NUM_TEST_PROC} --dist=${PYTEST_DIST_MODE} -s test && \
...
py-test:
    ...
    # - Specific test: make py-test ARGS=test/gbserver_test/utils/test_user_spaces_list.py
    # - Multiple args: make py-test ARGS="test/gbserver_test/api -k test_artifact_get"
	# $(MAKE) cicd-venv
	source $(VENVDIR)/bin/activate && \
		export GBTEST_ENABLE_EXTENDED_TESTS=false &&	\
		export GBSERVER_IMAGE_TAG=${IMAGE_TAG} && \
		export GBSERVER_SIDECAR_MONITORING_IMAGE_TAG=${SIDECAR_IMAGE_TAG} && \
		pytest -s $(or $(ARGS),test)

# Add new g4os test files to this list
G4OS_TEST_FILES = \
	test/gbserver_test/api/test_auth.py \
	test/gbserver_test/builtins/steps/test_s3pull.py \
	test/gbserver_test/builtins/steps/test_s3push.py \
	test/gbserver_test/commands/test_standalone_command.py \
	test/gbserver_test/environment/test_gpu_types.py \
	test/gbserver_test/environment/test_runpod.py \
	test/gbserver_test/environment/test_skypilot.py \
	test/gbserver_test/messaging/test_nats_messaging.py \
	test/gbserver_test/messaging/test_optional_rabbitmq.py \
	test/gbserver_test/spacesecretmanager/test_envspacesecretmanager.py \
	test/gbserver_test/spacesecretmanager/test_hybridspacesecretmanager.py \
	test/gbserver_test/test_optional_imports.py \
	test/gbserver_test/test_optional_lakehouse.py \
	test/gbserver_test/test_regression_smoke.py \
	test/gbserver_test/test_standalone_e2e.py \
	test/gbserver_test/test_standalone_rest_api_e2e.py
...
test-standalone:
	source $(VENVDIR)/bin/activate && pytest -s test/ -m 'not secret_manager and not nats_server and not docker_required and not ibm and not nats'
...
.PHONY: test-all
test-all:
	source $(VENVDIR)/bin/activate && pytest -s test/ -o "addopts="
