...
# Remove generated config files (regenerated by setup-slurm.sh on next run).
rm -f "$SCRIPT_DIR/slurm.conf" "$SCRIPT_DIR/gres.conf"

# ---- Step 2: Remove gbserver-managed and legacy blocks from ~/.slurm/config ----
# setup-slurm.sh no longer writes this file; gbserver materializes a
# `# BEGIN gbserver-managed (cluster config)` region from the inline
# environment.yaml at build time. Strip that block on teardown so a stale entry
# cannot collide with the next run's inline config. Also strip the *legacy*
# `# BEGIN slurm-docker (managed by setup-slurm.sh)` block written by older
# versions of this script — otherwise it lingers as foreign content and
# gbserver's refuse-on-conflict will reject the inline config.
...
strip_block() {  # $1 = begin marker, $2 = end marker
    if [ -f "$SLURM_SSH_CONFIG" ] && grep -qF "$1" "$SLURM_SSH_CONFIG"; then
        log "Removing managed block ('$1') from $SLURM_SSH_CONFIG"
