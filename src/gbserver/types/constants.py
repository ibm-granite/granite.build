...
# BlueVela remote-file REST API caps. SSH connection params are resolved
# per-request from the target's environment.yaml (via
# Environment.load_environment_config), not env vars.
ENV_VAR_GBSERVER_BLUEVELA_CONTENT_MAX_BYTES = (
    ENV_VAR_PREFIX + "_BLUEVELA_CONTENT_MAX_BYTES"
)
ENV_VAR_GBSERVER_BLUEVELA_DOWNLOAD_MAX_BYTES = (
    ENV_VAR_PREFIX + "_BLUEVELA_DOWNLOAD_MAX_BYTES"
)

# Defaults: 1 MiB for inline text, 500 MiB for streaming download.
BLUEVELA_CONTENT_MAX_BYTES = int(
    os.getenv(ENV_VAR_GBSERVER_BLUEVELA_CONTENT_MAX_BYTES, str(1 * 1024 * 1024))
)
BLUEVELA_DOWNLOAD_MAX_BYTES = int(
    os.getenv(ENV_VAR_GBSERVER_BLUEVELA_DOWNLOAD_MAX_BYTES, str(500 * 1024 * 1024))
)
