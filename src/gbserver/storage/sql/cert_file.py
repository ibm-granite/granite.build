...
from gbserver.types.constants import (
    ...
    GBSERVER_SQL_SSLROOT_CERT_FILE,
)
    ...
    if ssl_cert_file is not None:
        if ssl_cert_base64 is not None:
            if os.path.isfile(ssl_cert_file):
                logger.warning(
                    f"Both SQL ssl file and env var specified.  Using file specified {ssl_cert_file}"
                )
            else:
                logger.warning(
                    f"SQL ssl file {ssl_cert_file} does not exist.  Will use base64 env var"
                )
                ssl_cert_file = None
