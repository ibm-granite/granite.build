...
hfpull_acquire_lock() {
    local dest="$1" container lockdir ttl created now interval last_check
    ...
    while :; do
        ...
        if (( now - last_check >= interval )); then
            last_check="${now}"
            # A peer holds it. Reclaim only if past the ttl AND dest shows no
            # recent writes (a dead holder); otherwise keep waiting for the live
            # holder. Mirrors SharedFileSystemLock._clear_if_stale (info line 2 is
            # the holder's start time; fall back to the lock dir's mtime).
