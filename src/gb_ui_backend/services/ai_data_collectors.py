...
class DataCollector(ABC):
    @abstractmethod
    async def collect(
        self, session: AsyncSession, build: GbdBuild, context: BuildContext
    ) -> None:
        pass
    ...
    async def collect(
        ...
        if build.created_at:
            ...
            context.duration_str = (
                f"{h}h {m}m" if h > 0 else (f"{m}m {s}s" if m > 0 else f"{s}s")
            )
...
class ClusterQueueCollector(DataCollector):
    async def collect(
        ...
        stmt = (
            select(GbdClusterQueue)
            .where(
                GbdClusterQueue.cluster_name == build.cluster_name,
...
class PodLogCollector(DataCollector):
    ...
    async def collect(
        ...
        stmt = select(GbdK8sResource).where(
            GbdK8sResource.build_id == build.id, GbdK8sResource.kind == "Pod"
        )
        result = await session.execute(stmt)
        log_lines = []
...
class GbserverClient:
    ...
    async def get_build_data(
        self, build_id: str, max_events: int = 5000
        ...
        async with self._session_factory() as session:
                        ...
                        status_msgs.append(
                            {
                                ...
                                "name": row.name
                                or payload.get("definition_uri", "unknown"),
            ...
            def event_priority(e: Dict) -> tuple:
                if e["type"] == "MESSAGE_EVENT" and e.get("level") == "ERROR":
                    return (0, -e["index"])
                if e["type"] == "STATUS_EVENT" and e.get("status") in (
                    "FAILED",
                    "ERROR",
                ):
                    return (1, -e["index"])
...
class CloudLogsCollector(DataCollector):
    ...
    async def collect(
        ...
        try:
            ...
            if steps:
                tasks = [
                    self._fetch_step_logs(client, build_id_str, sid, sname)
                    for sid, sname in steps
                ]
                results = await asyncio.gather(*tasks)
                context.step_logs = {
                    name: content for name, content in results if content
                }
                ...
                response = await client.query_logs(
                    build_id=build_id_str, page_size=self.max_lines * 2
                )
    ...
    async def _fetch_step_logs(
        self, client: Any, build_id: str, step_id: str, step_name: str
    ) -> Tuple[str, str]:
        try:
            response = await client.query_logs(
                build_id=build_id, step_id=step_id, page_size=self.max_lines * 2
            )
...
class CompositeDataCollector:
        ...
        if collectors is not None:
            ...
            if gbserver_db_url:
                self.collectors.append(
                    GbserverDataCollector(
                        db_url=gbserver_db_url, schema=gbserver_db_schema
                    )
                )
            if cloud_logs_api_url and cloud_logs_api_key:
                self.collectors.append(
                    CloudLogsCollector(
                        api_url=cloud_logs_api_url,
                        api_key=cloud_logs_api_key,
                        max_lines=log_lines,
                    )
                )
            else:
    ...
    async def collect_build_context(
        self, session: AsyncSession, build: GbdBuild
    ) -> BuildContext:
        context = BuildContext(
            build_id=str(build.id), build_name=build.name, status=build.status
        )
...
async def query_knowledge_base(
    ...
    stmt = select(GbdMeta).where(
        or_(
            GbdMeta.human_solution.isnot(None),
        ...
        case_conditions.append(
            (
                and_(
                    ...
                    GbdMeta.error_category_2 == error_category_2,
                ),
                100,
            )
        )
