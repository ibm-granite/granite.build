...
```bash
cp docs/environments/setup/skypilot-setup.env.template .env.skypilot
```
...
```bash
source .env.skypilot
bash docs/environments/setup/setup-skypilot.sh
```
...
2. Set S3 bucket URIs in `configurations/spaces/local/space.yaml`:
   ```yaml
   variables:
...
- [SkyPilot on Kubernetes (`environment.yaml`)](../skypilot-kubernetes.md)
