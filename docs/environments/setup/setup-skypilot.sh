...
#
# Reads docs/environments/setup/skypilot-setup-config.yaml to resolve env vars with defaults,
...
#
# Usage:
#   1. Copy env template:   cp docs/environments/setup/skypilot-setup.env.template .env.skypilot
#   2. Edit your copy:      vi .env.skypilot
#   3. Source it:            source .env.skypilot
#   4. Run this script:     bash docs/environments/setup/setup-skypilot.sh
#
...
verify_sky_check() {
    ...
    if echo "${output}" | grep -qi "no gpu\|gpu.*not.*found\|gpu.*not.*available\|no accelerator"; then
        ...
        log_info "  - See: docs/environments/setup/skypilot-kubernetes-setup.md#label-gpu-nodes"
    fi
}
