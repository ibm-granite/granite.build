...
| `gbserver create-spaces [--spaces-path <f>] [--clear] [--replace] [--force]` | Create the predefined spaces (registers them in the `gb_spaces` table). Each space in the YAML accepts `name`, `git_repo_uri`, `lakehouse_namespace`, and an optional `resource_group_id` (the HF Enterprise resource group id to cache for HF pushes; no HF API lookup is done here). |
