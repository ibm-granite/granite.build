...
> not be injectable by arbitrary mid-stream output from cloned-repo code. Bash and Docker anchor
> it with a plain `^`; SkyPilot anchors it too but permits only its own `(name, pid=N) ` log
> prefix before the marker (`^(\([^)]*\)\s+)?(?:GB_|LLMB_)STEP_METADATA_KEY:...`), so a marker
